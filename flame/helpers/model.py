...
def create_model(
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        if device.type == "cuda":
            ...
            model = DistributedDataParallel(
                base_model, find_unused_parameters=find_unused_parameters
            )
...
def create_model_from_config(
    config: dict, find_unused_parameters: bool = False
) -> Model:
    """
    TODO: 应该拆分成两个函数
    """
    ...
    model = create_model(
        base_model, device, find_unused_parameters=find_unused_parameters
    )
    return model
...
def prepare_model(
    ...
    find_unused_parameters: bool = False,
    use_sync_bn: bool = False,
) -> Union[DistributedDataParallel, DataParallel]:
    base_model.to(device)
    ...
    if dist.is_available() and dist.is_initialized():
        ...
        if device.type == "cuda":
            ...
            _logger.warning(
                "You are using distributed model on cpu, maybe you are doing some test?"
            )
            model = DistributedDataParallel(
                base_model, find_unused_parameters=find_unused_parameters
            )
